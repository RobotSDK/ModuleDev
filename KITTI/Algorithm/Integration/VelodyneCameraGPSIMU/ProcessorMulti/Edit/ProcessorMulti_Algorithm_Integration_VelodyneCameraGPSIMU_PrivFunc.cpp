...
#include "../NoEdit/ProcessorMulti_Algorithm_Integration_VelodyneCameraGPSIMU_PrivFunc.h"
...
bool DECOFUNC(setParamsVarsOpenNode)(QString qstrConfigName, QString qstrNodeType, QString qstrNodeClass, QString qstrNodeName, void * paramsPtr, void * varsPtr)
{
	...
	ProcessorMulti_Algorithm_Integration_VelodyneCameraGPSIMU_Params * params=(ProcessorMulti_Algorithm_Integration_VelodyneCameraGPSIMU_Params *)paramsPtr;
	ProcessorMulti_Algorithm_Integration_VelodyneCameraGPSIMU_Vars * vars=(ProcessorMulti_Algorithm_Integration_VelodyneCameraGPSIMU_Vars *)varsPtr;
...
void DECOFUNC(initializeOutputData)(void * paramsPtr, void * varsPtr, boost::shared_ptr<void> & outputDataPtr)
{
	ProcessorMulti_Algorithm_Integration_VelodyneCameraGPSIMU_Params * params=(ProcessorMulti_Algorithm_Integration_VelodyneCameraGPSIMU_Params *)paramsPtr;
	ProcessorMulti_Algorithm_Integration_VelodyneCameraGPSIMU_Vars * vars=(ProcessorMulti_Algorithm_Integration_VelodyneCameraGPSIMU_Vars *)varsPtr;
	outputDataPtr=boost::shared_ptr<void>(new ProcessorMulti_Algorithm_Integration_VelodyneCameraGPSIMU_Data());
...
bool DECOFUNC(processMultiInputData)(void * paramsPtr, void * varsPtr, QVector<QVector<void *> > inputParams, QVector<QVector<void *> > inputData, void * outputData, QList<int> & outputPortIndex)
{
	...
	QVector<Simulator_Sensor_GPSIMU_Params *> inputparams_1; copyQVector(inputparams_1,inputParams[1]);
	QVector<ProcessorMulti_Algorithm_Integration_VelodyneCamera_Data *> inputdata_0; copyQVector(inputdata_0,inputData[0]);
	QVector<Simulator_Sensor_GPSIMU_Data *> inputdata_1; copyQVector(inputdata_1,inputData[1]);
	ProcessorMulti_Algorithm_Integration_VelodyneCameraGPSIMU_Data * outputdata=(ProcessorMulti_Algorithm_Integration_VelodyneCameraGPSIMU_Data *)outputData;
	...
	*/
	if(vars->init)
	{
		...
		inputparams_1[0]->rotation.copyTo(tmpR);
		inputparams_1[0]->translation.copyTo(tmpT);

		vars->tcalib=T.inv().t();

		vars->normalize=inputdata_1[0]->pose.inverse();
		int i,j;
		for(i=0;i<4;i++)
		{
			for(j=0;j<4;j++)
			{
				vars->tnormalize.at<float>(i,j)=float(vars->normalize(j,i));
			}
		}
	...
	outputdata->pose=vars->normalize*inputdata_1[0]->pose;
	...
	velpoints=velpoints*vars->tcalib*tpose*vars->tnormalize;
