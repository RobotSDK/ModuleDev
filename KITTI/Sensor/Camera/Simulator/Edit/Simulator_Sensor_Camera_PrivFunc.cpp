...
bool DECOFUNC(setParamsVarsOpenNode)(QString qstrConfigName, QString qstrNodeType, QString qstrNodeClass, QString qstrNodeName, void * paramsPtr, void * varsPtr)
{
	...
	*/
	...
	while(!file.atEnd())
	{
		...
		{
			params->size.width=int(values.at(0).toDouble()*vars->compressrate);
			params->size.height=int(values.at(1).toDouble()*vars->compressrate);
		}
		...
		{
			params->rectsize.width=int(values.at(0).toDouble()*vars->compressrate);
			params->rectsize.height=int(values.at(1).toDouble()*vars->compressrate);
		}
		...
		else if(prefix.startsWith(QString("P_rect_%1").arg(params->cameraid,2,10,QChar('0'))))
		{
			...
			params->rectprojection.at<float>(0,2)*=vars->compressrate;
			params->rectprojection.at<float>(1,2)*=vars->compressrate;
		}
	...
	vars->imagelist=dir.entryInfoList(QDir::Files | QDir::Hidden | QDir::NoSymLinks);
	
	if(vars->timestamps.size()!=vars->imagelist.size())
	{
		return 0;
	}
...
bool DECOFUNC(generateSourceData)(void * paramsPtr, void * varsPtr, void * outputData, QList<int> & outputPortIndex, QTime & timeStamp)
{
	...
	*/
	...
	outputdata->image=cv::imread(vars->imagelist.at(vars->index).absoluteFilePath().toStdString());
	if(vars->compressrate!=1)
	{
		cv::resize(outputdata->image,outputdata->image,params->rectsize);
	}
